...
export default function App() {
  ...
  const handleLoginSuccess = (user: any, token: string, refreshToken?: string) => {
    ...
    // Role-based screen auto-routing
    if (user?.role === 'ROLE_CHEF') {
      setActiveTab('chef_prep');
    } else if (user?.role === 'ROLE_MANAGER' || user?.role === 'ROLE_ADMIN') {
      setActiveTab('orders');
    } else {
  ...
  return (
    <div className="bg-[#09090B] text-slate-100 min-h-screen antialiased flex flex-col selection:bg-indigo-500 selection:text-white">
      ...
      <main className="flex-1">
        ...
        {activeTab === 'orders' && <PreBookingsDashboard orders={orders} currentUser={currentUser} />}
